...
const AddJadwalMapelModal = ({ open = false, closeModalHandler, type }) => {
  ...
  const create = async (event) => {
    ...
    try {
      ...
      await handleCreate("/jadwal-mapel", data);
      setLoading(false);
      alert("Berhasil mendaftarkan guru mapel");
      closeModalHandler();
      ...
      if (errorMsg) {
        openSnackBar(`Gagal mendaftarkan guru mapel: ${errorMsg}`);
        return;
      }
      openSnackBar("Gagal mendaftarkan guru mapel");
      return;
    }
  };
...
export default AddJadwalMapelModal;
