...
const MonthlyExportModal = ({
  ...
  const create = async (event) => {
    ...
    try {
      ...
      const month = moment(dailyValue).format("MM");
      const monthName = moment(dailyValue).format("MMM");
      const year = moment(dailyValue).format("YYYY");
      ...
      FileDownload(response.data, `rekap-absen-${monthName}-${year}.xlsx`);
      setLoading(false);
    } catch (error) {
      setLoading(false);
    }
